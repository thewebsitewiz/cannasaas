...
**Section 8 — Admin Portal: Full Implementation (All Subsections)**
**Version 3.0 | February 2026**
**Prepared for: Dennis Luken, Senior Architect / Site Lead**

> This document contains the complete **Section 8** of `CannaSaas-PublicBeta-DeepDive.md`,
> reconstructed in full. Sections 8.1–8.2 and 8.5 are preserved verbatim from the source file.
> Sections 8.3 and 8.4 were re-authored from `api-reference.md`, `compliance-guide.md`, and
> `architecture.md`. Section 8.6 was authored fresh using `multi-tenancy.md`, `architecture.md`,
> and the `onboarding` module specification (Sprint 7).
...
The onboarding wizard is the first experience a new dispensary manager has after their account is provisioned. It guides them through seven steps — from business identity to compliance credentials — in a linear flow with persistent progress. Completing the wizard results in a fully configured tenant: schema provisioned, branding applied, Metrc connected, and the dispensary ready to receive inventory.

The wizard lives at `/admin/onboarding` and is protected by the `OnboardingGuard`, which redirects newly-provisioned organizations here before they can access any other admin page. Once `organization.onboardingComplete === true`, the guard dissolves and the user is redirected to `/dashboard`.
