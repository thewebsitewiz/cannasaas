...
| 4   | ~~`orders.service.ts` is 1,000+ lines mixing tax math, stock reserve, status transitions~~                                              | ✅       | —      | done     | Split into `OrderCreatorService` (createOrder + tax/THC math), `OrderStateMachineService` (confirm/preparing/ready/complete/cancel), and `OrderQueryService` (read paths). `OrdersService` is now a 119-line facade so resolvers stay untouched. Shared row types live in `order-types.ts`. Partial work shipped earlier (PR #141) extracted the common helpers into `order-helpers.ts`. |
...
| 11  | ~~Two test runners (Vitest in Angular, Jest in API)~~                                                                                  | ✅       | —      | done     | API is now 100% Vitest — both `test` and `test:e2e` scripts use it. Closed by dropping the TypeORM entity glob (`DirectoryExportedClassesLoader`) in favor of an explicit `ALL_ENTITIES` list barrel at `apps/api/src/entities.index.ts` consumed by both `DatabaseModule` and `AppDataSource`, plus removing the migrations glob from `DatabaseModule` (only the CLI's `data-source.ts` runs migrations). New regression guard `test/integration/entities-barrel.e2e-spec.ts` fails CI if a `.entity.ts` file is silently missing from the barrel. Jest, ts-jest, @types/jest + both jest configs deleted. |
