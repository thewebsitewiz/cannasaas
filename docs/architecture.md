...
| 11  | Two test runners (Vitest in Angular, Jest in API) — **partial: unit suite migrated**                                                  | 🟡       | M      | P3       | API unit suite runs under Vitest as of the #11-partial PR (418/418 tests green via `pnpm --filter @cannasaas/api test`). E2E suite stays on Jest pending follow-up: TypeORM's `DirectoryExportedClassesLoader` glob-`require()`s `.entity.ts` files at runtime, and Vitest's loader doesn't register a CJS extension hook for TS the way `ts-jest` does. Two ways to close the remainder — (a) refactor `DatabaseModule` to pass entities explicitly instead of via glob, or (b) point the e2e config at a built `dist/`. Both are dedicated work, not piggy-back-able. |
