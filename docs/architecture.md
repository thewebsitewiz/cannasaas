...
| 6   | Font catalog duplicated server vs. admin app                                                                                           | 🟡       | S      | P2       | Move to `packages/types`. §7 #6.                                                                                             |
| 7   | Public-path matching is `startsWith` (open to subpath bypass)                                                                          | 🟡       | XS     | P2       | Use exact-match regex. §7 #5.                                                                                                |
| 8   | `validateFile` cap duplicated in `uploadBranding`                                                                                      | 🟡       | XS     | P2       | Parameterize `validateFile(file, maxSize)`. §7 #3.                                                                           |
...
| 10  | Angular admin's no-tenant-theming CLAUDE.md rule blocks rollout of sc-637 to admin/staff/kiosk                                         | 🟡       | S      | P1       | Next theme PR flips this rule.                                                                                               |
| 11  | Two test runners (Vitest in Angular, Jest in API)                                                                                      | 🔵       | XL     | P3       | Long-term standardize on Vitest.                                                                                             |
| 12  | Event-name magic strings scattered                                                                                                     | 🔵       | S      | P3       | Consolidate to `common/events/event-names.ts`. §7 #10.                                                                       |
| 13  | Inline 600-line component templates in `theme-page.ts`                                                                                 | 🔵       | M      | P3       | Split into child standalone components. §7 #9.                                                                               |
| 14  | `metrc.resolver.spec` has pre-existing DI failures (`MetrcLicenseValidatorService`)                                                    | 🟡       | S      | P2       | Surfaced during last 2 weeks; needs the missing provider in test setup.                                                      |
| 15  | `CreateRegisterSessions` migration FK pattern (sc-709 fixed) had been silently swallowing similar FK errors in `AddForeignKeyCascades` | 🟡       | M      | P1       | PR #123 noted: existing dev/prod DBs may be missing customer/employee/refresh-token cascade FKs. Follow-up migration needed. |
