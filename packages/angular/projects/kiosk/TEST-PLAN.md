...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AuthService` ✅             | Token precedence (device > access > null + both-set), `ensureLoggedIn` (device, cached access, fresh login, inflight dedup), `KIOSK_NOT_PROVISIONED` (no env creds, network error, empty response), 401 retry contract (`clearAccessToken` device-only no-op, access-only clear, both-tokens no-op, `clearToken()` alias), token mutators (`setDeviceToken` + empty-no-op + clears cached access; `clearDeviceToken` preserves access), `canSelfLogin` env-driven                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Automated — 20 specs at [`src/app/core/auth/auth.service.spec.ts`](src/app/core/auth/auth.service.spec.ts). The device-only `clearAccessToken` no-op is the critical safeguard: a single server-side 401 must not wipe the kiosk's long-lived provisioning.                                                                   |
| `CartService` ✅             | `addItem` (new + increment), `updateQuantity` (set, zero-to-remove, negative-to-remove, unknown-id no-op), `removeItem` (match + unknown-id no-op), `clearCart`, `itemCount` (empty + cross-variant sum), `subtotal` (empty + price × qty + decimal pricing). Plus customer-state lifecycle.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Automated — 22 specs at [`src/app/core/cart/cart.service.spec.ts`](src/app/core/cart/cart.service.spec.ts). No localStorage persistence (the cart is intentionally in-memory so it clears on idle/attract reset between customers, per `clearCart` lifecycle); the previous "localStorage round-trip" note was stale.         |
| `IdleService` ✅             | start() registers pointerdown/touchstart/keydown (idempotent), stop() removes them (idempotent), start→stop→start re-registers cleanly. Pre-idle: timeout boundary fires exactly at N ms, activity halfway through pushes the fire out. Post-idle: further activity is intentionally ignored (only AttractMode's `reset()` clears it); `reset()` restarts the timer; `reset()` without start() is a safe no-op.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Automated — 12 specs at [`src/app/core/idle/idle.service.spec.ts`](src/app/core/idle/idle.service.spec.ts). Fake timers via `vi.useFakeTimers()` per TEST-PLAN guidance; DOCUMENT mocked via `provide: DOCUMENT, useValue: { addEventListener, removeEventListener }`.                                                        |
| `AttractMode` (component) ✅ | Initial render on SLIDES[0]; exposes the full 3-slide deck. Rotation interval (5000ms): no advance pre-boundary, advances exactly at boundary, continues across multiple intervals, wraps at end of deck (2 → 0, not 3), continuous over a long soak (10 cycles, no drift). Tap-to-exit: emits `exit` on host click + on touchstart (real kiosk hardware); emits per-gesture (no debounce — parent handles re-entry). Destroy: clears the rotation interval (no leak after teardown).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Automated — 11 specs at [`src/app/shared/attract-mode/attract-mode.spec.ts`](src/app/shared/attract-mode/attract-mode.spec.ts). Fake timers via `vi.useFakeTimers()`. AttractMode is presentational; the "renders when isIdle" and "clears cart + resets" parts belong to the parent (KioskLayout / IdleService integration). |
| `MenuPage` ✅                | Filter signal: initial `All` filter sends `productTypeId: null`; setting Vape triggers a new fetch with `productTypeId: 3`. Loading branch: `loading()` true while the observable hasn't emitted. Empty branch: `filtered() === []` when fetch returns `[]`. Data branch: returns the products array. Error branch (sc-733): `loading()` flips false, `error()` exposes the Error, `filtered()` stays empty (no throw); template renders error UI with Retry; `reload()` re-runs the fetch and clears the error; non-Error throws surface as Errors via rxResource wrapping. `goToProduct` navs to `/product/:id`. `onAddClick` adds first variant + flashes `addedId` for 1500ms; a second add of a different product takes over the flash; no-op when product has no variants. `priceOf` / `isInStock` (stockQuantity > 0; null-stockQuantity falls back to stockStatus). `addControlClasses` distinct strings for sold-out / added / default.                                                                                                               | Automated — 17 specs at [`src/app/pages/menu/menu-page.spec.ts`](src/app/pages/menu/menu-page.spec.ts). Requires `fixture.detectChanges()` after construction to wire `rxResource`'s reactive params effect. Error-handling fix shipped under sc-733.                                                                         |
| `ProductPage` ✅             | Product resolution (fetch with route `:id` + dispensaryId; `of(null)` stream when no id; loading until emit; null product when not found). Error branch (sc-733): `loading()` flips false, `error()` exposes the Error, `product()` stays null (no throw); template renders error UI with Retry; `reload()` re-runs the fetch and clears the error; non-Error throws surface as Errors via rxResource wrapping. `activeVariant` defaults to `variants[0]`, honors `selectVariant`, falls back to `[0]` when selected id no longer in list. `selectVariant` resets qty to 1; no-op when target is sold out. `inc()` caps at stockQuantity; `dec()` floors at 1. `add()` loops `qty` times into cart (collapses to one line via CartService.addItem increment); caps loop defensively at stock; flashes `added` → resets + navigates to `/` after 1500ms; no-op when no product / sold out. Helpers: `priceFor` (0 fallback for null retailPrice), `isVariantInStock`, `stockFor` (floors + clamps null to 0), `effectsOf`/`flavorsOf` coerce jsonb to string[]. | Automated — 24 specs at [`src/app/pages/product/product-page.spec.ts`](src/app/pages/product/product-page.spec.ts). Error-handling fix shipped under sc-733.                                                                                                                                                                  |
| `CheckoutPage` ✅            | Empty-cart guard: `placeOrder` is a no-op (no auth, no mutate, no nav, no loading flip). Happy path: mutate called with `dispensaryId` + `KIOSK_ORDER_DEFAULTS` + lineItems; walk-in omits `customerUserId`; checked-in customer attaches it. On success: `cart.clearCart()` + nav to `/confirm/:orderId`. Auth gateway (401-retry surface): `auth.ensureLoggedIn()` called BEFORE mutate; `KIOSK_NOT_PROVISIONED` short-circuits with error + cart preserved. Errors: network/GraphQL error surface + cart preserved; missing `orderId` → "Order failed — please try again"; non-Error throws fall back to "Order failed"; next attempt clears the prior error. Re-entry guard: second `placeOrder()` while still loading is a no-op (single mutation).                                                                                                                                                                                                                                                                                                       | Automated — 12 specs at [`src/app/pages/checkout/checkout-page.spec.ts`](src/app/pages/checkout/checkout-page.spec.ts). The actual 401 retry lives one layer down in `auth-retry-link.ts` (Apollo link); CheckoutPage's contract is just "call ensureLoggedIn first, surface whatever error bubbles up."                      |
| `OrderConfirmPage` ✅        | Initial state (countdown=15, shortOrderId uppercased 8 chars, '—' fallback when route has no orderId). Countdown ticks (1s decrement, partial-second no-op). Auto-redirect at zero (`router.navigateByUrl('/')` fires once + interval cleared so no further ticks). Manual reset (`goHome()` navigates; interval intentionally keeps ticking until destroy — the consumer is navigating away). Destroy clears the interval (no nav, no further ticks).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Automated — 10 specs at [`src/app/pages/order-confirm/order-confirm-page.spec.ts`](src/app/pages/order-confirm/order-confirm-page.spec.ts). Fake timers via `vi.useFakeTimers()`.                                                                                                                                             |
| `KioskLayout` ✅             | Back button: `isHome` true on `/`, false on non-root, reactively flips on each `NavigationEnd` (both directions). Cart badge: 0 on empty, mirrors `CartService.itemCount` after add, returns to 0 after clear. Reset button: clears items + customer + navigates to `/`. `goBack()` delegates to `Location.back()`. Customer pill: exposes `CartService.customer` (null = "Check In" template branch).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Automated — 11 specs at [`src/app/layouts/kiosk-layout/kiosk-layout.spec.ts`](src/app/layouts/kiosk-layout/kiosk-layout.spec.ts). Targets class methods + computed signals directly to avoid full RouterLink template wiring; Router mocked via `{ events: Subject, url, navigateByUrl }`, Location via `{ back }`.           |
...
### 6.9b Device attestation — `X-Device-Signature` (sc-474)

ECDSA P-256 with a non-extractable CryptoKey persisted in IndexedDB. Legacy kiosks (no `kiosk_devices.public_key`) bypass; once attested, every request must be signed.

| ID         | Case                                                                | Steps                                                                                                                                        | Expected                                                                                                                          |
| ---------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| TC-ATT-001 | Setup attests fresh kiosk                                           | Wipe IDB (`indexedDB.deleteDatabase('cs.kiosk.attestation')`). Visit `/setup?token=DEV_DEVICE_TOKEN`.                                        | `attestKioskDevice` fires once with SPKI PEM. localStorage has the token; IDB `cs.kiosk.attestation` has a key. Navigate to `/`.  |
| TC-ATT-002 | Subsequent requests carry the header                                | After TC-ATT-001, watch any GraphQL request in DevTools.                                                                                     | Request includes `X-Device-Signature: <iat-ms>.<32-hex-nonce>.<base64url-sig>`. iat is within ±60 s of `Date.now()`.              |
| TC-ATT-003 | Stolen JWT alone is useless                                         | Copy the JWT from localStorage; paste it as Authorization header on a separate machine that doesn't have the IDB key. Issue any kiosk query. | 401 from `KioskAttestationGuard` (`Missing X-Device-Signature`).                                                                  |
| TC-ATT-004 | Stolen JWT + stolen signature = single replay window                | Capture one signed request. Replay the exact same request body + signature.                                                                  | First replay succeeds (within freshness window). Second replay rejects: `Replayed nonce`.                                         |
| TC-ATT-005 | Body substitution rejected                                          | Capture a signed request. Replay with a swapped body inside the freshness + nonce window.                                                    | 401: `Bad device signature`.                                                                                                      |
| TC-ATT-006 | Stale iat rejected                                                  | Capture a signed request; wait 65 s; replay.                                                                                                 | 401: `Signature iat outside freshness window`.                                                                                    |
| TC-ATT-007 | Cleared IDB falls back to unsigned (and gets rejected)              | Clear site data on an attested kiosk. Issue any kiosk query.                                                                                 | 401 (`Missing X-Device-Signature`); operator re-provisions via `/setup`.                                                          |
| TC-ATT-008 | Re-provisioning invalidates the prior attestation                   | Provision kiosk A (gets key). Re-provision the same `(dispensary, label)` via `provisionKiosk`. Try a kiosk A request with its old key.      | 401 (key mismatch — server `current_token_id` rotated AND `public_key` cleared). Operator re-runs `/setup` to attest a fresh key. |
| TC-ATT-009 | Legacy kiosk (no kiosk_devices row) keeps working                   | Authenticate as the seeded `kiosk@greenleaf.com` service account. Issue any kiosk query without an `X-Device-Signature` header.              | Succeeds. Legacy bypass remains.                                                                                                  |
| TC-ATT-010 | Pre-attestation token can call `attestKioskDevice` but nothing else | After provisioning, issue any non-attest mutation BEFORE running `attestKioskDevice`.                                                        | Succeeds (`public_key IS NULL` bypass). Once `attestKioskDevice` lands, the second non-attest call must be signed.                |
| TC-ATT-011 | Second `attestKioskDevice` rejected                                 | Run `attestKioskDevice` twice with different keys without re-provisioning in between.                                                        | Second call returns `ConflictException: Device is already attested; re-provision to rotate the key`.                              |
| TC-ATT-012 | Cross-device IDB cannot be exfiltrated via DevTools                 | On a real Android tablet, open DevTools and inspect the IDB store.                                                                           | The stored `CryptoKey` object is opaque — no exportable key material is visible. `subtle.exportKey('pkcs8', key)` rejects.        |
| TC-ATT-013 | Hardware soak — IDB persists across reload                          | Provision + attest. Hard-reload 10 times over 5 minutes. Issue queries between reloads.                                                      | IDB key persists; every request stays signed. No re-attestation.                                                                  |
