...
| `MenuPage` ✅             | Filter signal: initial `All` filter sends `productTypeId: null`; setting Vape triggers a new fetch with `productTypeId: 3`. Loading branch: `loading()` true while the observable hasn't emitted. Empty branch: `filtered() === []` when fetch returns `[]`. Data branch: returns the products array. Error branch: loading transitions to false; `filtered()` throws (known gap — `filtered = computed(() => resource.value() ?? [])` only coalesces undefined, not error). `goToProduct` navs to `/product/:id`. `onAddClick` adds first variant + flashes `addedId` for 1500ms; a second add of a different product takes over the flash; no-op when product has no variants. `priceOf` / `isInStock` (stockQuantity > 0; null-stockQuantity falls back to stockStatus). `addControlClasses` distinct strings for sold-out / added / default. | Automated — 15 specs at [`src/app/pages/menu/menu-page.spec.ts`](src/app/pages/menu/menu-page.spec.ts). Requires `fixture.detectChanges()` after construction to wire `rxResource`'s reactive params effect. Latent error-handling gap filed as a follow-up. |
| `ProductPage` ✅          | Product resolution (fetch with route `:id` + dispensaryId; `of(null)` stream when no id; loading until emit; null product when not found; **same rxResource error gap as MenuPage** — `product()` throws on error, tracked in sc-733). `activeVariant` defaults to `variants[0]`, honors `selectVariant`, falls back to `[0]` when selected id no longer in list. `selectVariant` resets qty to 1; no-op when target is sold out. `inc()` caps at stockQuantity; `dec()` floors at 1. `add()` loops `qty` times into cart (collapses to one line via CartService.addItem increment); caps loop defensively at stock; flashes `added` → resets + navigates to `/` after 1500ms; no-op when no product / sold out. Helpers: `priceFor` (0 fallback for null retailPrice), `isVariantInStock`, `stockFor` (floors + clamps null to 0), `effectsOf`/`flavorsOf` coerce jsonb to string[]. | Automated — 22 specs at [`src/app/pages/product/product-page.spec.ts`](src/app/pages/product/product-page.spec.ts). Same rxResource pattern as MenuPage — error gap also tracked under sc-733. |
