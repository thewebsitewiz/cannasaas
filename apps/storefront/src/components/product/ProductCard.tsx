...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const price = firstVariant?.retailPrice
  ...
  const inCart = cartItems.find(
    (i) =>
      ...
      i.variantId === (firstVariant?.variantId ?? product.id),
  );
  const atMax =
    stockQuantity != null && inCart != null && inCart.quantity >= stockQuantity;
  ...
  return (
    ...
    >
      ...
      <div className="aspect-square bg-gradient-to-br from-brand-50 to-brand-100 flex items-center justify-center relative">
        ...
        {product.strainType && (
          <span
            className={`absolute top-3 left-3 text-[10px] font-semibold px-2 py-0.5 rounded-full border ${STRAIN_COLORS[product.strainType] ?? STRAIN_COLORS.hybrid}`}
          >
            {product.strainType}
          </span>
        )}
        ...
        {!isOutOfStock && (
          <button
            ...
            aria-label={
              atMax
                ? `${product.name} — maximum quantity in cart`
