      ...
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <ShoppingBag size={18} />
            {itemCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 bg-emerald-500 text-white text-[9px] font-bold rounded-full w-4.5 h-4.5 min-w-[18px] min-h-[18px] flex items-center justify-center shadow-sm">
                {itemCount}
              </span>
            )}
          </Link>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 shadow-lg">
          <nav className="max-w-7xl mx-auto px-6 py-4 flex flex-col gap-1">
            {['Menu', 'Flower', 'Edibles', 'Vapes', 'Pre-Rolls', 'Express Checkout'].map((item) => (
              <Link
                key={item}
                href={item === 'Menu' ? '/products' : item === 'Express Checkout' ? '/express-checkout' : `/products?category=${item.toLowerCase()}`}
