...
export default function OrderTrackingPage() {
  ...
  useEffect(() => {
    if (orderId) {
      gqlAuth<{ order: Order | null }>(ORDER_QUERY, {
        id: orderId,
        ...
        .catch(() => {
          // ignore — show "Order not found" UI
        })
  ...
  useEffect(() => {
    if (lastUpdate && lastUpdate.orderId === orderId) {
      setOrder((prev) =>
        prev ? { ...prev, orderStatus: lastUpdate.status } : prev,
      );
    }
  }, [lastUpdate, orderId]);
  ...
  if (loading) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center text-txt-muted">
        Loading...
      </div>
    );
  }
  ...
  const isPickup =
    order.orderType === 'pickup' || order.orderType === 'in_store';
        ...
        {!isCancelled && (
          <div className="mt-8">
            <div className="flex items-center justify-between relative">
              ...
              {STEPS.filter((_, i) => (isPickup ? i !== 3 : true)).map(
                (step, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`mt-2 text-xs font-medium ${isActive ? 'text-brand-600' : 'text-txt-muted'}`}
                      >
                        {step.label}
                      </span>
                    </div>
