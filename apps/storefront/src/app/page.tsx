    ...
    <div className="overflow-hidden">
      ...
      <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden">
        ...
        <div className={`relative z-10 max-w-5xl mx-auto px-6 text-center transition-all duration-1000 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <h1 className="text-5xl sm:text-7xl lg:text-8xl font-light text-white tracking-tight leading-[0.9] mb-6" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
            Curated
            <br />
            <span className="italic text-emerald-300">Cannabis</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link
              href="/products"
              className="group inline-flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-white font-medium px-8 py-4 rounded-full transition-all duration-300 hover:shadow-lg hover:shadow-emerald-500/20"
            >
              Browse Menu
            ...
            <Link
              href="/products"
              className="inline-flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 text-white/80 hover:text-white font-medium px-8 py-4 rounded-full border border-white/10 transition-all duration-300 backdrop-blur-sm"
            >
              View Strains
            </Link>
          </div>
        ...
        {/* Scroll indicator */}
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 text-white/20 animate-bounce" style={{ animationDuration: '3s' }}>
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          {CATEGORIES.map((cat, i) => (
            <Link
              ...
              className={`group bg-white rounded-2xl p-5 text-center border border-gray-100 hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-500/5 transition-all duration-500 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
              style={{ transitionDelay: `${i * 80 + 800}ms` }}
            >
              <div className={`w-12 h-12 mx-auto rounded-xl bg-gradient-to-br ${cat.gradient} flex items-center justify-center text-2xl mb-3 group-hover:scale-110 transition-transform duration-300`}>
                {cat.emoji}
              </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { icon: Truck, title: 'Free Local Delivery', desc: 'Complimentary delivery within our service area. Orders placed before 5pm arrive same day.', accent: 'emerald' },
            { icon: ShieldCheck, title: 'Lab Tested & Tracked', desc: 'Every product verified through state compliance. Full COA available for each batch.', accent: 'teal' },
            { icon: Clock, title: 'Skip the Line', desc: 'Order ahead and pick up in minutes. Express checkout for returning customers.', accent: 'green' },
          ].map((prop, i) => (
            <div
              key={prop.title}
              className={`group p-8 rounded-2xl bg-gradient-to-b from-gray-50 to-white border border-gray-100 hover:border-emerald-100 transition-all duration-500 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
              style={{ transitionDelay: `${i * 100 + 1200}ms` }}
            >
              <div className={`w-12 h-12 rounded-xl bg-${prop.accent}-50 flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-300`}>
                <prop.icon size={22} className={`text-${prop.accent}-600`} />
              </div>
      ...
      {products.length > 0 && (
        <section className="max-w-7xl mx-auto px-6 pb-24">
          <div className="flex items-end justify-between mb-10">
            ...
            <Link
              href="/products"
              className="group hidden sm:flex items-center gap-1.5 text-sm font-medium text-emerald-600 hover:text-emerald-700 transition-colors"
            >
              View all
      ...
      <section className="bg-[#0a1a0f] text-white py-24">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <Link
            href="/register"
            className="inline-flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-white font-medium px-8 py-4 rounded-full transition-all duration-300 hover:shadow-lg hover:shadow-emerald-500/20"
          >
            Create Account
            <ArrowRight size={16} />
          </Link>
