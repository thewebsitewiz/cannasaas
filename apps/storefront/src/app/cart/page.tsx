...
export default function CartPage() {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-txt-muted mb-6">
          Browse our menu and add some products
        </p>
        <Link
          href="/products"
          className="inline-block bg-brand-600 text-txt-inverse px-6 py-3 rounded-lg font-semibold hover:bg-brand-500 transition-colors"
        >
          Browse Menu
        </Link>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      <h1 className="text-2xl font-bold text-txt mb-6">
      ...
      <div className="lg:grid lg:grid-cols-[1fr_18rem] lg:gap-8">
          ...
          {items.map((item: CartItem) => {
            ...
            return (
              <div
                ...
                className="bg-surface rounded-xl border border-bdr p-4 flex items-center gap-4"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.strainType && (
                    <span className="text-xs text-txt-faint capitalize">
                      {item.strainType}
                    </span>
                  )}
                  {atMax && (
                ...
                <div className="flex items-center gap-1 border border-bdr rounded-lg">
                  ...
                  <span className="px-2 text-sm font-medium tabular-nums w-8 text-center text-txt">
                    {item.quantity}
                  </span>
                  <button
                    onClick={() =>
                  ...
                  >
                    <Plus
                      size={14}
                      className={atMax ? 'text-txt-faint' : 'text-txt-muted'}
                    />
                  </button>
                </div>
                <p className="text-sm font-bold tabular-nums w-16 text-right text-txt">
                  ${(item.price * item.quantity).toFixed(2)}
                </p>
                <button
                  onClick={() => removeItem(item.productId)}
                  className="p-1.5 text-txt-muted hover:text-danger transition-colors"
                >
                  <Trash2 size={16} />
                </button>
              </div>
        ...
        <aside className="mt-6 lg:mt-0">
          <div className="bg-surface rounded-xl border border-bdr p-6 sticky top-24">
            ...
            <Link
              href="/checkout"
              className="mt-4 w-full flex items-center justify-center gap-2 bg-brand-600 text-txt-inverse py-3 rounded-lg font-semibold hover:bg-brand-500 transition-colors"
            >
