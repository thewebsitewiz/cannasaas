...
export default function ExpressCheckoutPage() {
  ...
  const { data: lastOrder } = useQuery({
    ...
    queryFn: () =>
      gqlAuth<{ myLastOrder: LastOrder | null }>(LAST_ORDER_QUERY, {
  ...
  const { data: favorites } = useQuery({
    ...
    queryFn: () =>
      gqlAuth<{ myFavorites: Favorite[] | null }>(FAVORITES_QUERY, {
        dispensaryId: DEFAULT_DISPENSARY_ID,
      }),
  ...
  if (!token) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <Link
          href="/login?redirect=/express-checkout"
  ...
  const handleExpressOrder = async () => {
    ...
    const items = favorites
      .filter((f) => selectedFavorites.includes(f.productId))
      .map((f) => ({
  ...
  const placeOrder = async (lineItems: OrderInput[], orderType: string) => {
    setLoading(true);
    setError('');
    try {
      const data = await gqlAuth<{
        createOrder: { orderId: string } | null;
      }>(CREATE_ORDER, {
  ...
  const estimatedTime =
    lastOrder?.orderType === 'delivery' ? '45-60 min' : '~15 min';
              ...
              {lastOrder.lineItems?.map((li, i) => (
                <div key={i} className="flex justify-between text-sm">
                  ...
                  <span className="tabular-nums font-medium">
                    ${Number(li.price * li.quantity).toFixed(2)}
                  </span>
                </div>
              ))}
            ...
            <div className="flex flex-wrap gap-4 mb-4 text-sm text-gray-500">
              {lastOrder.shippingAddress && (
                <div className="flex items-center gap-1.5">
                  ...
                  {lastOrder.shippingAddress.street},{' '}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-gray-500">
                          {f.variantName} &middot; Ordered {f.orderCount}x
                        </p>
                      </div>
                    </div>
        ...
        {!lastOrder && (!favorites || favorites.length === 0) && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mb-4">
              No order history yet. Place your first order to unlock express
              checkout!
            </p>
            <Link
              href="/products"
