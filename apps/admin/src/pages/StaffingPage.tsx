...
export function StaffingPage() {
  ...
  if (isLoading) return <div className="text-txt-muted p-8">Loading staff...</div>;
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-txt">Staffing</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <div className="bg-surface rounded-xl border border-border p-4 text-center">
          ...
          <p className="text-xs text-txt-secondary">Expiring Soon</p>
        </div>
        <div className="bg-surface rounded-xl border border-border p-4 text-center">
          ...
          <p className="text-xs text-txt-secondary">Expired</p>
        </div>
        <div className="bg-surface rounded-xl border border-border p-4 text-center">
          <p className="text-2xl font-bold">${totalPayroll.toLocaleString()}</p>
          <p className="text-xs text-txt-secondary">Est. Weekly Payroll</p>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-xl border border-border overflow-hidden">
        <div className="px-6 py-4 border-b border-border">
          <h2 className="text-lg font-semibold text-txt">Employee Roster</h2>
        </div>
        <table className="w-full text-sm">
          <thead className="bg-bg-alt border-b border-border">
            <tr>
              ...
              <th className="text-left px-4 py-2 font-medium text-txt-secondary">Type</th>
              <th className="text-right px-4 py-2 font-medium text-txt-secondary">Rate</th>
              <th className="text-center px-4 py-2 font-medium text-txt-secondary">Certs</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {(employees ?? []).map((emp: any) => (
              <tr key={emp.profileId}
                className="hover:bg-bg-alt cursor-pointer transition-colors"
              ...
              >
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-medium text-txt">{emp.firstName} {emp.lastName}</p>
                      <p className="text-xs text-txt-muted">{emp.employeeNumber} · {emp.email}</p>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-4 py-3 text-center">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${emp.employmentStatus === 'active' ? 'bg-green-50 text-green-700' : 'bg-danger-bg text-danger'}`}>
