...
export class LoggingInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    let req: any = null;
    try { req = context.switchToHttp().getRequest(); } catch { /* GraphQL/WS context */ }

    const requestId = req?.headers?.['x-request-id'] || randomUUID();
    const method = req?.method || (contextType === 'graphql' ? 'GQL' : contextType);
    const url = req?.url || '';
    const now = Date.now();
    ...
    return next.handle().pipe(
      tap(() => {
        ...
        if (contextType !== 'graphql') this.logger.log(`${method} ${url} ${ms}ms [${requestId}]`);
