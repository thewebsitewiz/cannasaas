...
export class RecommendationResolver {
  ...
  async recommendations(
    ...
    return this.recService.getFrequentlyBoughtTogether(
      productId,
  ...
  async popularInCategory(
    ...
    return this.recService.getPopularInCategory(
      categoryId,
      this.resolveDispensary(user, dispensaryId),
      limit ?? 10,
    );
  ...
  async trending(
    ...
    return this.recService.getTrendingProducts(
      this.resolveDispensary(user, dispensaryId),
      days ?? 7,
      limit ?? 10,
    );
  ...
  async personalizedForMe(
    ...
    return this.recService.getPersonalizedForCustomer(
      user.sub,
      this.resolveDispensary(user, dispensaryId),
      limit ?? 10,
    );
