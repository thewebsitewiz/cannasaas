...
@Resolver()
export class CanPayOnboardingResolver {
  constructor(private readonly onboarding: CanPayOnboardingService) {}
  ...
  @Mutation(() => DispensaryPaymentProcessor, {
    name: 'provisionCanPayForDispensary',
  })
  provision(
    @Args('input') input: ProvisionCanPayInput,
  ...
  @Roles('dispensary_admin', 'org_admin', 'super_admin')
  @Mutation(() => Boolean, { name: 'deprovisionCanPayForDispensary' })
  async deprovision(
