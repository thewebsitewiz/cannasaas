...
export class BiotrackService {
  ...
  async upsertCredential(input: {
    ...
    if (!BIOTRACK_STATES.includes(input.state)) {
      throw new BadRequestException(
        `BioTrack is not available in state: ${input.state}. Supported: ${BIOTRACK_STATES.join(', ')}`,
      );
    }

    const encryptedApiKey = this.encrypt(input.apiKey);
    const encryptedApiSecret = input.apiSecret
      ? this.encrypt(input.apiSecret)
      : undefined;
  ...
  async validateCredential(
    ...
    if (!credential)
      throw new NotFoundException(
        'No BioTrack credential found for this dispensary',
      );
    ...
    try {
      ...
      this.logger.log(
        `BioTrack credential validated for ${dispensaryId} (state: ${credential.state})`,
      );
      return {
        valid: true,
        message: `BioTrack ${credential.state} credential validated (stub)`,
      };
    } catch (err: unknown) {
  ...
  async syncInventory(
    ...
    if (!credential)
      throw new NotFoundException('No active BioTrack credential');
    ...
    return {
      success: true,
      message: 'Inventory sync queued (stub)',
      itemCount: 0,
    };
  }
  ...
  async reportSale(
    ...
    const orderRowsRaw = (await this.dataSource.query(
    ...
    this.logger.log(
      `BioTrack sale reported: order=${orderId} dispensary=${dispensaryId}`,
    );
    return {
      success: true,
      message: `Sale ${orderId} reported to BioTrack (stub)`,
    };
  }
  ...
  async getComplianceSystem(dispensaryId: string): Promise<string> {
    const biotrack = await this.credentialRepo.findOne({
      where: { dispensaryId, isActive: true },
    });
    if (biotrack) return 'biotrack';

    const metrcRowsRaw = (await this.dataSource.query(
      `SELECT credential_id FROM metrc_credentials WHERE dispensary_id = $1 AND is_active = true LIMIT 1`,
      [dispensaryId],
    )) as unknown;
    const metrcRows = metrcRowsRaw as Array<{ credential_id: string }>;
    if (metrcRows[0]) return 'metrc';
