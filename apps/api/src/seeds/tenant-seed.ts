...
async function seed() {
  ...
  try {
    ...
    const companyId =
      companyResult[0]?.company_id ??
      (
        await qr.query(
          `SELECT company_id FROM companies WHERE organization_id = $1 LIMIT 1`,
          [orgId],
        )
      )[0].company_id;
    ...
    const dispensaryId =
      dispResult[0]?.entity_id ??
      (
        await qr.query(
          `SELECT entity_id FROM dispensaries WHERE slug = $1 LIMIT 1`,
          ['green-leaf-nyack'],
        )
      )[0].entity_id;
    ...
    await qr.query(
      ...
      ON CONFLICT (email) DO UPDATE SET password_hash = EXCLUDED.password_hash, dispensary_id = EXCLUDED.dispensary_id, organization_id = EXCLUDED.organization_id
    `,
    ...
    const products = [
      {
        name: 'Blue Dream',
        type_code: 'FLOWER',
        cat_code: 'FLOWER_HYBRID',
        thc: 22.5,
        cbd: 0.3,
        price: 29.99,
        desc: 'A classic sativa-dominant hybrid with sweet berry aroma.',
      },
      {
        name: 'OG Kush Pre-Roll',
        type_code: 'PRE_ROLL',
        cat_code: 'PREROLL_SINGLE',
        thc: 24.0,
        cbd: 0.1,
        price: 12.99,
        desc: 'Premium OG Kush ground flower in a 1g pre-roll.',
      },
      {
        ...
        cbd: 0.5,
        price: 49.99,
        desc: 'Live resin 510 cartridge with sweet strawberry terpenes.',
      },
      {
        name: 'Mango Gummies',
        type_code: 'EDIBLE',
        cat_code: 'EDIBLE_GUMMY',
        thc: 5.0,
        cbd: 0.0,
        price: 24.99,
        desc: '10-pack mango flavored gummies, 5mg THC each.',
      },
      {
        name: 'Wedding Cake',
        type_code: 'FLOWER',
        cat_code: 'FLOWER_INDICA',
        thc: 25.0,
        cbd: 0.2,
        price: 39.99,
        desc: 'Rich and tangy with earthy pepper notes.',
      },
    ];

    for (const p of products) {
      const typeRows = await qr.query(
        `SELECT product_type_id FROM lkp_product_types WHERE code = $1`,
        [p.type_code],
      );
      const catRows = await qr.query(
        `SELECT category_id FROM lkp_product_categories WHERE code = $1`,
        [p.cat_code],
      );
      ...
      `,
        [
          dispensaryId,
          typeRows[0]?.product_type_id,
          catRows[0]?.category_id,
          p.name,
          p.desc,
          p.thc,
          p.cbd,
