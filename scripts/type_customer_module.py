#!/usr/bin/env python3
"""
Type the customer module: replaces raw `any` typing with proper row-shape
interfaces. Idempotent — re-running produces the same output.

Run from repo root:
    python3 scripts/type_customer_module.py
"""
from pathlib import Path

SERVICE_PATH = Path("apps/api/src/modules/customers/customer.service.ts")
RESOLVER_PATH = Path("apps/api/src/modules/customers/customer.resolver.ts")

SERVICE = r"""import { Injectable, Logger, NotFoundException, ConflictException } from '@nestjs/common';
...
export class CustomerService {
  ...
  async createCustomerProfile(userId: string, input: CreateCustomerProfileInput): Promise<CustomerProfile> {
  ...
  async verifyAge(userId: string, input: VerifyAgeInput): Promise<{ verified: boolean; age: number; reason?: string }> {
  ...
  async addAddress(userId: string, input: AddressInput): Promise<CustomerAddress> {
    if (input.isDefault) {
  ...
  async updateAddress(addressId: string, userId: string, input: AddressInput): Promise<CustomerAddress> {
  ...
  ): Promise<{ allowed: boolean; limit?: number; remaining?: number; reason?: string }> {
    ...
    for (const rule of rules) {
      ...
      if (rule.period_type === 'transaction') {
        if (quantityGrams > maxGrams) {
          return { allowed: false, limit: maxGrams, remaining: maxGrams, reason: rule.description ?? undefined };
        }
        ...
        const totalGrams = parseFloat(purchasedRows[0]?.total_grams ?? '0') || 0;
        const remaining = maxGrams - totalGrams;

        if (totalGrams + quantityGrams > maxGrams) {
          return { allowed: false, limit: maxGrams, remaining: Math.max(0, remaining), reason: rule.description ?? undefined };
        }
      }
    }
...
RESOLVER = r"""import { Resolver, Query, Mutation, Args, ID, Int, Float, ObjectType, Field, InputType } from '@nestjs/graphql';
import { ForbiddenException } from '@nestjs/common';
import { CustomerService, CustomerProfileWithUserRow, OrderHistoryRow } from './customer.service';
import { KioskCustomerLookup } from './dto/kiosk-customer-lookup.type';
import { CustomerProfile, CustomerAddress, AgeVerification } from './entities/customer.entity';
...
export class CustomerResolver {
  ...
  async myProfile(@CurrentUser() user: JwtPayload): Promise<CustomerProfileWithUserRow | null> {
...
def main() -> None:
    if not SERVICE_PATH.exists():
        raise SystemExit(f"missing: {SERVICE_PATH} (run from repo root)")
    if not RESOLVER_PATH.exists():
        raise SystemExit(f"missing: {RESOLVER_PATH} (run from repo root)")

    SERVICE_PATH.write_text(SERVICE)
    print(f"✓ wrote {SERVICE_PATH} ({len(SERVICE)} bytes)")

    RESOLVER_PATH.write_text(RESOLVER)
    print(f"✓ wrote {RESOLVER_PATH} ({len(RESOLVER)} bytes)")

    print("\nNext:")
    print("  pnpm --filter @cannasaas/api lint")
    print("  pnpm --filter @cannasaas/api build    # type-check sanity")
